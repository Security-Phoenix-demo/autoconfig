...
def create_environment(environment, headers, verify_only=False):
    print("[Environment]")
...
def add_environment_services(repos, subdomains, environments, application_environments, phoenix_components, subdomain_owners, teams, access_token, verify_only=False):
        ...
        if environment['Services']:  # Updated key to Services
            for service in environment['Services']:
                ...
                if not environment_service_exist(env_id, phoenix_components, service['Service']):
                    try:
                        if team_name:
                            services.append(add_service(env_name, service['Service'], service['Tier'], team_name, headers, verify_only))
                        else:
                            services.append(add_service(env_name, service['Service'], service['Tier'], headers, verify_only))
                    except NotImplementedError as e:
                        print(f"Error adding service {service['Service']} for environment {env_name}: {e}")
                
                service_rules.extend(add_service_rule_batch(environment, service, headers, verify_only))

    return {"services": services, "service_rules": service_rules}
...
def add_service_rule_batch(environment, service, headers, verify_only=False):
    payload = None
    ...
    if service.get('Tag'):
        try:
            ...
            rules.append(create_component_rule(environment['Name'], service['Service'], 'tags', [{"key": tag_parts[0], "value": tag_parts[1]}], f"Rule for tags for {service['Service']}", headers, verify_only))
        except Exception as e:
            print(f"Error: Parsing tag for {service['Service']} failed. Expected 'key:value', got {service['Tag']}")
        
    if service.get('SearchName'):
        rules.append(create_component_rule(environmentName, serviceName, 'keyLike', service['SearchName'], f"Rule for keyLike for {serviceName}", headers, verify_only))
    if service.get('Fqdn'):
        rules.append(create_component_rule(environmentName, serviceName, 'fqdn', service['Fqdn'], f"Rule for fqdn for {serviceName}", headers, verify_only))
    if service.get('Netbios'):
        rules.append(create_component_rule(environmentName, serviceName, 'netbios', service['Netbios'], f"Rule for netbios for {serviceName}", headers, verify_only))
    if service.get('OsNames'):
        rules.append(create_component_rule(environmentName, serviceName, 'osNames', service['OsNames'], f"Rule for osNames for {serviceName}", headers, verify_only))
    if service.get('Hostnames'):
        rules.append(create_component_rule(environmentName, serviceName, 'hostnames', service['Hostnames'], f"Rule for hostnames for {serviceName}", headers, verify_only))
    if service.get('ProviderAccountId'):
        rules.append(create_component_rule(environmentName, serviceName, 'providerAccountId', service['ProviderAccountId'], f"Rule for providerAccountId for {serviceName}", headers, verify_only))
    if service.get('ProviderAccountName'):
        rules.append(create_component_rule(environmentName, serviceName, 'providerAccountName', service['ProviderAccountName'], f"Rule for providerAccountName for {serviceName}", headers, verify_only))
    if service.get('ResourceGroup'):
        rules.append(create_component_rule(environmentName, serviceName, 'resourceGroup', service['ResourceGroup'], f"Rule for resourceGroup for {serviceName}", headers, verify_only))
    if service.get('AssetType'):
        rules.append(create_component_rule(environmentName, serviceName, 'assetType', service['AssetType'], f"Rule for assetType for {serviceName}", headers, verify_only))

    if service.get('MultiConditionRule'):
        rules.extend(create_multicondition_service_rules(environmentName, serviceName, [service.get('MultiConditionRule')], headers, verify_only))
    if service.get('MultiConditionRules'):
        rules.extend(create_multicondition_service_rules(environmentName, serviceName, service.get('MultiConditionRules'), headers, verify_only))
...
def create_applications(applications, application_environments, phoenix_components, headers, verify_only=False):
    print('[Applications]')
    verify_data = {'created': [], 'updated': []}
    for application in applications:
        if not any(env['name'] == application['AppName'] and env['type'] == "APPLICATION" for env in application_environments):
            verify_data['created'].append(create_application(application, headers, verify_only))
        else:
            verify_data['updated'].append(update_application(application, application_environments, phoenix_components, headers, verify_only))
    return verify_data

def create_application(app, headers, verify_only=False):
    verify_data = {'application': {}, 'components': []}
    payload = {
        "name": app['AppName'],
        "type": "APPLICATION",
    ...
    if verify_only:
          verify_data['application'] = payload
    else:
        if DEBUG:
            print(f"Payload being sent to /v1rule: {json.dumps(payload, indent=2)}")
    ...
    for component in app['Components']:
        verify_data['components'].append(create_custom_component(app['AppName'], component, headers, verify_only))
    
    return verify_data

def create_custom_component(applicationName, component, headers, verify_only=False):
    verify_data = {'component': {}, 'component_rules': []}
    ...
    verify_data['component_rules'] = create_component_rules(applicationName, component, headers, verify_only)
    return verify_data

def update_application(application, existing_apps_envs, existing_components, headers, verify_only=False):
    verify_data = {'teams': [], 'crit_owner': {}, 'components_created': [], 'components_updated': [], 'component_rules': []}
    ...
    verify_data['teams'] = update_application_teams(existing_app, application, headers, verify_only)

    verify_data['crit_owner'] = update_application_crit_owner(application, existing_app, headers, verify_only)
    ...
    for component in application['Components']:
        ...
        if not existing_component:
            verify_data['components_created'].append(create_custom_component(application['AppName'], component, headers, verify_only))
            continue

        verify_data['components_updated'].append(update_component(application, component, existing_component, headers, verify_only))
        verify_data['component_rules'].append(create_component_rules(application['AppName'], component, headers, verify_only))
    return verify_data

def update_component(application, component, existing_component, headers, verify_only=False):
    verify_data = {'teams_removed': [], 'tags_removed': [], 'tags_added': []}
    for team in filter(lambda tag: tag.get('key') == 'pteam', existing_component.get('tags')):
        if team.get('value') not in component.get('TeamNames'):
            verify_data['teams_removed'].append(remove_tag_from_component(team.get('id'), team.get('key'), team.get('value'), existing_component.get('id'), headers, verify_only))
    # Ensure valid tag values by filtering out empty or None 
    tags_to_remove = []
    tags_to_add = []
    ...
    for tag in tags_to_remove:
        ...
        verify_data['tags_removed'].append(remove_tag_from_component(tag.get('id'), tag.get('key'), tag.get('value'), existing_component.get('id'), headers, verify_only))
    ...
    if verify_only:
        verify_data['tags_added'] = payload
        return verify_data
...
def update_application_teams(existing_app, application, headers, verify_only=False):
    verify_data = {'removed': [], 'added': []}
    for team in filter(lambda tag: tag.get('key') == 'pteam', existing_app.get('tags')):
        if team.get('value') not in application.get('TeamNames'):
            verify_data['removed'].append(remove_tag_from_application(team.get('id'), team.get('key'), team.get('value'), existing_app.get('id'), headers, verify_only))
    ...
    for new_team in application.get('TeamNames'):
        if not next(filter(lambda team: team.get('key') == 'pteam' and team['value'] == new_team, existing_app.get('tags')), None):
            verify_data['added'].append(add_tag_to_application('pteam', new_team, existing_app.get('id'), headers, verify_only))
    return verify_data

def update_application_crit_owner(application, existing_application, headers, verify_only=False):
...
def create_component_rules(applicationName, component, headers, verify_only=False):
    rules = []
    if component.get('SearchName'):
        rules.append(create_component_rule(applicationName, component['ComponentName'], 'keyLike', component['SearchName'], f"Rule for keyLike for {component['ComponentName']}", headers, verify_only))
    if component.get('Tags'):
        tags_to_add = []
        for tag in component.get('Tags'):
            tags_to_add.append({'value': tag})
        rules.append(create_component_rule(applicationName, component['ComponentName'], 'tags', tags_to_add, f"Rule for tags for {component['ComponentName']}", headers, verify_only))
    if component.get('Cidr'):
        rules.append(create_component_rule(applicationName, component['ComponentName'], 'cidr', component['Cidr'], f"Rule for cidr for {component['ComponentName']}", headers, verify_only))
    if component.get('Fqdn'):
        rules.append(create_component_rule(applicationName, component['ComponentName'], 'fqdn', component['Fqdn'], f"Rule for fqdn for {component['ComponentName']}", headers, verify_only))
    if component.get('Netbios'):
        rules.append(create_component_rule(applicationName, component['ComponentName'], 'netbios', component['Netbios'], f"Rule for netbios for {component['ComponentName']}", headers, verify_only))
    if component.get('OsNames'):
        rules.append(create_component_rule(applicationName, component['ComponentName'], 'osNames', component['OsNames'], f"Rule for osNames for {component['ComponentName']}", headers, verify_only))
    if component.get('Hostnames'):
        rules.append(create_component_rule(applicationName, component['ComponentName'], 'hostnames', component['Hostnames'], f"Rule for hostnames for {component['ComponentName']}", headers, verify_only))
    if component.get('ProviderAccountId'):
        rules.append(create_component_rule(applicationName, component['ComponentName'], 'providerAccountId', component['ProviderAccountId'], f"Rule for providerAccountId for {component['ComponentName']}", headers, verify_only))
    if component.get('ProviderAccountName'):
        rules.append(create_component_rule(applicationName, component['ComponentName'], 'providerAccountName', component['ProviderAccountName'], f"Rule for providerAccountName for {component['ComponentName']}", headers, verify_only))
    if component.get('ResourceGroup'):
        rules.append(create_component_rule(applicationName, component['ComponentName'], 'resourceGroup', component['ResourceGroup'], f"Rule for resourceGroup for {component['ComponentName']}", headers, verify_only))
    if component.get('AssetType'):
        rules.append(create_component_rule(applicationName, component['ComponentName'], 'assetType', component['AssetType'], f"Rule for assetType for {component['ComponentName']}", headers, verify_only))

    if component.get('MultiConditionRule'):
        rules.extend(create_multicondition_component_rules(applicationName, component['ComponentName'], [component.get('MultiConditionRule')], headers, verify_only))

    if component.get('MultiConditionRules'):
        rules.extend(create_multicondition_component_rules(applicationName, component['ComponentName'], component.get('MultiConditionRules'), headers, verify_only))

    repository_names = component.get('RepositoryName', [])
    if isinstance(repository_names, str):
        repository_names = [repository_names]
    for repo_name in repository_names:
        rules.append(create_component_rule(applicationName, component['ComponentName'], 'repository', [repo_name], f"Rule for repository for {component['ComponentName']}", headers, verify_only))

    return rules

            
# Handle Repository Rule Creation for Components
def create_component_rule(applicationName, componentName, filterName, filterValue, ruleName, headers, verify_only=False):
    rule = {
        "name": ruleName,
...
def create_multicondition_component_rules(applicationName, componentName, multiconditionRules, headers, verify_only=True):
    verify_data_rules = []
    for multicondition in multiconditionRules:
        ...
        if verify_only:
            verify_data_rules.append(payload)
            continue
        ...
        try:
            ...
            if response.status_code == 409:
                ...
                print(f"Response content: {response.content}")
                exit(1)
    return verify_data_rules

def create_multicondition_service_rules(environmentName, serviceName, multiconditionRules, headers, verify_only=False):
    verify_data_rules = []
    for multicondition in multiconditionRules:
...
def add_cloud_asset_rules(repos, access_token, verify_only=False):
    headers = {'Authorization': f"Bearer {access_token}", 'Content-Type': 'application/json'}
    asset_rules = []
    # Loop through each repository and modify domain if needed
    for repo in repos:
        search_term = f"*{repo['RepositoryName']}(*"
        asset_rules.append(cloud_asset_rule(repo['Subdomain'], search_term, "Production", access_token, verify_only))

    return asset_rules

# CloudAssetRule Function
def cloud_asset_rule(name, search_term, environment_name, access_token, verify_only=False):
    headers = {'Authorization': f"Bearer {access_token}", 'Content-Type': 'application/json'}
...
def create_teams(teams, pteams, access_token, verify_only=False):
    """
    ...
    for team in teams:
        ...
        if not found and team['TeamName']:
            ...
            if verify_only:
                verify_data.append(payload)
                payload['id'] = '123'
                new_pteams.append(payload)
                continue
            ...
            try:
                ...
                if response.status_code == 400:
                    ...
                    print(f"Error: {e}")
                    exit(1)
    return {"new_pteams": new_pteams, "verify_data": verify_data}


def create_teams_from_pteams(applications, environments, pteams, access_token, verify_only=False):
    ...
    teams_resp = create_teams(teams_to_add, pteams, access_token, verify_only)
    create_team_rules(teams_to_add, pteams, access_token, verify_only)
    return teams_resp.get("verify_data", [])
...
def create_team_rules(teams, pteams, access_token, verify_only=False):
    """
    ...
    for team in teams:
        ...
        for pteam in pteams:
            if pteam['name'] == team['TeamName']:
                ...
                if team.get('RecreateTeamAssociations'):
                    ...
                    verify_data.append(create_team_rule("pteam", team['TeamName'], pteam['id'], access_token, verify_only))
                found = True
                break
        ...
        if not found and team['TeamName']:
            ...
            verify_data.append(create_team_rule("pteam", team['TeamName'], team.get('id', ''), access_token, verify_only))
    
    return verify_data

def create_team_rule(tag_name, tag_value, team_id, access_token, verify_only=False):
    """
...
@dispatch(list,list,list,list,list,str, bool)
def assign_users_to_team(p_teams, new_pteams, teams, all_team_access, hive_staff, access_token, verify_only=False):
    """
...
def remove_tag_from_application(tag_id, tag_key, tag_value, application_id, headers, verify_only=False):
    """
    Removes the specified tag by making a DELETE or PATCH API call.
...
def remove_tag_from_component(tag_id, tag_key, tag_value, component_id, headers, verify_only=False):
    """
...
def add_tag_to_application(tag_key, tag_value, application_id, headers, verify_only=False):
    """
...
@dispatch(str, str, int, dict, bool)
def add_service(applicationSelectorName, service, tier, headers, verify_only=False):
...
@dispatch(str, str, int, str, dict, bool)
def add_service(applicationSelectorName, service, tier, team, headers, verify_only=False):
    criticality = calculate_criticality(tier)
...
def does_member_exist(email, team, headers, verify_only=False):
    """
    Check if a member with a specific email exists in the given team.
    """
    if verify_only:
        # skip checks for verify mode
        return True
    try:
...
def add_thirdparty_services(phoenix_components, application_environments, subdomain_owners, headers, verify_only):
    verify_data = []
    services = [
    ...
    for service in services:
        if not environment_service_exist(env_id, phoenix_components, service):
            verify_data.append(add_service(env_name, service, 5, "Thirdparty", subdomain_owners, headers, verify_only))
    return verify_data
...
def create_deployments(applications, environments, phoenix_apps_envs, headers, verify_only=False):
...
def create_autolink_deployments(applications, environments, headers, verify_only=False):
    deployments = []
...
def create_components_from_assets(applicationEnvironments, phoenix_components, headers, verify_only=False):
    verify_data = []
            ...
            if appEnv.get('type') == "ENVIRONMENT":
                ...
                for group in asset_groups:
                    if len(group) > ASSET_GROUP_MIN_SIZE_FOR_COMPONENT_CREATION and not group[0] in already_suggested_components\
                        ...
                        verify_data.append(create_custom_component(appEnv['name'], component_to_create, headers, verify_only))
