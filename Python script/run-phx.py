...
def save_auto_created_components_to_yaml(auto_created_components, output_file=None):
    ...
    # Structure the YAML data to match core-structure.yaml format
    yaml_data = {
        '# Auto-generated by Phoenix AutoConfig': None,
        '# Generated at': datetime.now().isoformat(),
        '# Description': 'Auto-created components and services from environment assets',
        'AllAccessAccounts': [],
        'DeploymentGroups': [],
        'Environment Groups': []
    }
    
    # Group components by assignment strategy
    deployment_groups = {}  # For application components
    environment_groups = {}  # For environment services
    ...
    for component in auto_created_components:
        ...
        if assignment_strategy == 'component':
            # Add to DeploymentGroups (Application Components)
            if app_name not in deployment_groups:
                deployment_groups[app_name] = {
                    'AppName': app_name,
                    'Status': 'Production',  # Default status
                    'Deployment_set': app_name.lower().replace(' ', '-'),
                    'ReleaseDefinitions': [],
                    'Responsable': 'auto-generated@phoenix.com',
                    'Tier': 5,
                    'TeamNames': team_names if team_names else ['auto-generated'],
                    'Components': []
                }
            
            # Add component data with metadata
            component_data = {
                'ComponentName': f"phx-auto-{component_name}",
                'Status': 'Production',
                'Tier': 5,
                'TeamNames': team_names if team_names else ['auto-generated']
            }
            
            # Add tag-based metadata if applicable (removed for cleaner YAML)
            
            deployment_groups[app_name]['Components'].append(component_data)
            
        else:
            # Add to Environment Groups (Environment Services)
            if env_name not in environment_groups:
                environment_groups[env_name] = {
                    'Name': env_name,
                    'Type': component.get('environment_subtype', 'CLOUD').upper(),
                    'Status': 'Production',
                    'Responsable': component.get('responsible_user', 'auto-generated@phoenix.com'),
                    'Tier': 5,
                    'Tag_label': f'Environment:Production',
                    'TeamName': team_names[0] if team_names else 'auto-generated',
                    'Services': []
                }
            
            # Create MULTI_MultiConditionRules for asset association
            multi_condition_rules = []
            asset_names = component.get('asset_names', [])
            
            if asset_names:
                # Create rules based on asset names for SearchName
                # Include ALL assets in the group for complete coverage
                for asset_name in asset_names:  # Include all assets, not just first 5
                    rule = {
                        'SearchName': asset_name
                    }
                    multi_condition_rules.append(rule)
            
            # Add service data with metadata and rules
            service_data = {
                'Service': f"phx-auto-{component_name}",
                'Type': component.get('service_type', 'Cloud'),
                'Deployment_set': component_name.lower().replace(' ', '-')
            }
            
            # Add MULTI_MultiConditionRules if we have asset associations
            if multi_condition_rules:
                service_data['MULTI_MultiConditionRules'] = multi_condition_rules
            
            environment_groups[env_name]['Services'].append(service_data)
    
    # Convert to lists for YAML output
    yaml_data['DeploymentGroups'] = list(deployment_groups.values())
    yaml_data['Environment Groups'] = list(environment_groups.values())
    ...
    try:
        ...
        # Show summary
        deployment_groups_count = len(yaml_data['DeploymentGroups'])
        environment_groups_count = len(yaml_data['Environment Groups'])
        components_count = sum(len(dg['Components']) for dg in yaml_data['DeploymentGroups'])
        services_count = sum(len(eg['Services']) for eg in yaml_data['Environment Groups'])
        
        print(f"📊 Summary: {len(auto_created_components)} items total")
        print(f"   📦 {components_count} Application Components in {deployment_groups_count} Deployment Groups")
        print(f"   🔧 {services_count} Environment Services in {environment_groups_count} Environment Groups")
...
def perform_actions(args, config_file_path):  
    ...
    action_create_components_from_assets_tag = args.action_create_components_from_assets_tag == 'true'
    ...
    # Extract CLI parameters for component creation
    cli_asset_type = getattr(args, 'type', None)
    cli_tag_base = getattr(args, 'tag_base', None)
    cli_tag_alternative = getattr(args, 'tag_alternative', None)
    cli_create_automatically_groups = getattr(args, 'create_automatically_groups', 'no') == 'yes'
    cli_silent = getattr(args, 'silent', False)
    ...
    if action_cloud:
        ...
        try:
            ...
            for repo in repos:
                track_operation('cloud_assets', 'add_environment_service', repo['RepositoryName'], True)
        except Exception as e:
            for repo in repos:
                track_operation('cloud_assets', 'add_environment_service', repo['RepositoryName'], False, str(e))
    ...
    if action_create_components_from_assets or action_create_components_from_assets_tag:
        # Determine grouping method and action name
        if action_create_components_from_assets_tag:
            grouping_method = 'tag_based'
            action_name = 'Create Components from Assets (Tag-Based)'
            print("Performing create components/services from assets using TAG-BASED grouping")
        else:
            grouping_method = 'name_based'
            action_name = 'Create Components from Assets (Name-Based)'
            print("Performing create components/services from assets using NAME-BASED grouping")
        
        execution_report['actions_performed'].append(action_name)
        
        try:
            # Create CLI configuration override
            cli_config = {
                'asset_type': cli_asset_type,
                'grouping_method': grouping_method,
                'tag_base': cli_tag_base,
                'tag_alternative': cli_tag_alternative,
                'create_automatically_groups': cli_create_automatically_groups,
                'silent': cli_silent
            }
            
            if cli_create_automatically_groups:
                print("\n🚀 FULL ASSET CREATION MODE ENABLED")
                print("   This will create: Environments → Services → Applications → Components")
                
            auto_created_components = create_components_from_assets(app_environments, phoenix_components, headers, cli_config)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--action_create_components_from_assets", type=str, default="false", 
                        required=False, help="Flag triggering create components from assets action (name-based grouping)")
    parser.add_argument("--action_create_components_from_assets_tag", type=str, default="false",
                        required=False, help="Flag triggering create components from assets action (tag-based grouping)")
    parser.add_argument("--type", type=str, choices=["all", "cloud", "code", "infrastructure", "web"], 
                        default=None, required=False, help="Asset type to process (all, cloud, code, infrastructure, web)")
    parser.add_argument("--tag-base", type=str, default=None,
                        required=False, help="Base tag key for tag-based grouping (e.g., 'team-name')")
    parser.add_argument("--tag-alternative", type=str, default=None,
                        required=False, help="Alternative tag keys for tag-based grouping (comma-separated, e.g., 'pteam,owner')")
    parser.add_argument("--create_automatically_groups", type=str, choices=["yes", "no"], default="no",
                        required=False, help="Create automatically groups - full Phoenix structure (environments, services, applications, components)")
    parser.add_argument("--silent", action="store_true", help="Skip confirmation prompts and auto-accept creation")
